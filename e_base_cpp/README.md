# e_base_cpp
#### 概要
1. c代码包含.cpp与.h文件，.h文件只包含声明，.cpp文件是实现；这里只能通过libxx.a的方式进行引入
#### c中几个常见问题
...
6. <font color=red>c++中的很多包，需要用#cgo CFLAGS: -Iinclude目录  但是c++包引用的c++类的写法可能不满足c语言的风格，导致引入失败，所以尽量使用c包或封装c包的c++包</font>
